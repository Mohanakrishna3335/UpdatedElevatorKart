...
//jdbc connection
public class ConnectedElevators {

	public List<Elevator> ConnectedelevatorData() {
		...
		try {
			...
			ResultSet rs = stmt
					.executeQuery("select  eno,ename,features  from koneiot.elevators where etype='Connected';");
			while (rs.next()) {
				eno = rs.getInt("eno");
				elevator.setEno(eno);
